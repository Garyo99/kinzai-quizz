(() => {
  ...
  const requestedQuestion = window.QUESTIONS.find(
    (question) => question.id === params.get("question"),
  );
  let pendingQuestionId = requestedQuestion?.id || null;
  const state = {
    subject:
      requestedQuestion?.subject ||
      (["財務", "税務", "法務"].includes(requestedSubject)
        ? requestedSubject
        : "財務"),
    format: requestedQuestion?.format || "all",
    review: false,
    queue: [],
    index: 0,
    answered: false,
  ...
  function loadCheckLater() {
    try {
      return new Set(JSON.parse(localStorage.getItem(CHECK_LATER_KEY)) || []);
    } catch {
      return new Set();
    }
  }
  function saveCheckLater(items) {
    localStorage.setItem(CHECK_LATER_KEY, JSON.stringify([...items]));
  }
  function legacyAnswers(record) {
    ...
    const priorWrong = Math.max(
      0,
      attempts - correctCount - (lastCorrect ? 0 : 1),
    );
    const results = [
      ...Array(priorWrong).fill(false),
      ...Array(priorCorrect).fill(true),
    ];
    if (attempts) results.push(lastCorrect);
    return results.map((correct, index) => ({
      correct,
      answeredAt: index === attempts - 1 ? record.lastAnsweredAt : null,
    }));
  }
  function matching() {
    return window.QUESTIONS.filter(
      (q) =>
        ...
        (state.format === "all" || q.format === state.format),
    );
  }
  ...
  function updateStats() {
    ...
    const subjectItems = window.QUESTIONS.filter(
    ...
    const checkLater = loadCheckLater();
    els.answered.textContent = subjectItems.filter((q) => checkLater.has(q.id)).length;
    els.accuracy.textContent = records.length
      ? `${Math.round((correct / records.length) * 100)}%`
      : "—";
  ...
  function buildQueue(doShuffle = true) {
    ...
    let items = matching();
    if (state.review)
      items = items.filter((q) => history[q.id] && !history[q.id].lastCorrect);
    state.queue = doShuffle ? shuffle(items) : items;
    if (pendingQuestionId) {
      const requestedIndex = state.queue.findIndex(
        (question) => question.id === pendingQuestionId,
      );
      if (requestedIndex > 0)
        state.queue.unshift(...state.queue.splice(requestedIndex, 1));
  ...
  function render() {
    ...
    $("open-chatgpt").hidden = true;
    els.streakTag.hidden = true;
    if (!q) {
      els.quiz.hidden = true;
      els.empty.hidden = false;
      els.emptyMessage.textContent = state.review
        ? "この条件で復習待ちの問題はありません。"
        : "この条件に合う問題がありません。";
      return;
    }
    ...
    els.typeTag.textContent = q.sourceType;
    els.formatTag.textContent = `${q.format}択`;
    ...
    const checkedLater = loadCheckLater().has(q.id);
    $("check-later").classList.toggle("active", checkedLater);
    $("check-later").setAttribute("aria-pressed", checkedLater);
    $("check-later").textContent = checkedLater
      ? "確認リスト追加済み"
      : "あとで確認";
  ...
  function answer(selected) {
    ...
    [...els.choices.children].forEach((button, index) => {
      ...
      if (index === q.answer) {
        ...
        button.setAttribute("aria-label", `正解: ${q.options[index]}`);
      } else if (index === selected) {
        button.classList.add("wrong");
        button.setAttribute("aria-label", `不正解: ${q.options[index]}`);
      }
    });
    const history = loadHistory();
    const old = history[q.id] || { attempts: 0, correct: 0 };
    ...
    history[q.id] = {
      ...
      answers: [...legacyAnswers(old), { correct: isCorrect, answeredAt }],
    };
    saveHistory(history);
    updateStats();
  ...
  function setFormat(value) {
    ...
    document.querySelectorAll(".format").forEach((b) => {
      const active = b.dataset.format === String(value);
  ...
  function setMode(review) {
    state.review = review;
    [$("normal-mode"), $("review-mode")].forEach((b, i) => {
      const active = review ? i === 1 : i === 0;
      b.classList.toggle("active", active);
      b.setAttribute("aria-pressed", active);
    });
    buildQueue();
  }
  $("normal-mode").addEventListener("click", () => setMode(false));
  $("review-mode").addEventListener("click", () => setMode(true));
  $("back-all").addEventListener("click", () => setMode(false));
  $("shuffle-button").addEventListener("click", () => buildQueue(true));
  $("check-later").addEventListener("click", () => {
    const q = state.queue[state.index];
    if (!q) return;
    const items = loadCheckLater();
    if (items.has(q.id)) items.delete(q.id);
    else items.add(q.id);
    saveCheckLater(items);
    const active = items.has(q.id);
    const button = $("check-later");
    button.classList.toggle("active", active);
    button.setAttribute("aria-pressed", active);
    button.textContent = active ? "確認リスト追加済み" : "あとで確認";
    updateStats();
  });
  ...
  $("copy-question").addEventListener("click", async () => {
    ...
    const button = $("copy-question");
    button.textContent = "コピーしました";
    window.setTimeout(() => {
      if (state.answered) button.textContent = "この問題をコピー";
    }, 1800);
  });
  $("open-chatgpt").addEventListener("click", async () => {
    if (await copyPrompt()) window.location.href = "https://chatgpt.com/";
  });
  ...
  $("reset-history").addEventListener("click", () => {
    if (confirm("すべての回答履歴を削除しますか？この操作は元に戻せません。")) {
      localStorage.removeItem(STORAGE_KEY);
      localStorage.removeItem(CHECK_LATER_KEY);
      setMode(false);
    }
  });
  document.addEventListener("keydown", (e) => {
    ...
    else if (state.answered && (e.key === "Enter" || e.key === "ArrowRight"))
      els.next.click();
  });
  document
    .querySelectorAll(".subject")
    .forEach((button) =>
      button.classList.toggle(
        "active",
        button.dataset.subject === state.subject,
      ),
    );
  document.querySelectorAll(".format").forEach((button) => {
    const active = button.dataset.format === String(state.format);
    button.classList.toggle("active", active);
    button.setAttribute("aria-pressed", active);
  });
